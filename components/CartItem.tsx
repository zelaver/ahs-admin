...
const CartItem = memo(({ image, name, price = 0, val, setVal, setTotal, total, stok, editable = true }: CartItem) => {
  return (
    <View className={`cart-item mb-4 px-5 ${!val && "hidden"}`}>
      <View className="flex-row justify-between border-b border-gray-500 py-4">
        ...
        <View className="gambarBarang-jumlah w-[108px] justify-center">
          ...
          <View className={`jumlah flex-row items-center ${editable ? "justify-between" : "justify-center"}`}>
            {editable && (
              <TouchableOpacity
